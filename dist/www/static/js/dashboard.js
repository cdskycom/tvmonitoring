...
var vm = new Vue({
	...
	methods:{
		...
		handletopage:function(playload){
				...
				var url = '/api/toubleticket/gettask?page=' + this.tasks.currentPage + '&items_perpage=' + 
	...
	//初始化页面数据
	created: function () {
    	var url = '/api/toubleticket/statistic?' + 'uid=' + userinfo.uid + '&pid=' + userinfo.pid;
    	that = this
		axios.get(url).then(function(res){
		...
		var taskUrl = '/api/toubleticket/gettask?page=' + this.tasks.currentPage + '&items_perpage=' + 
			this.tasks.itemsPerPage + '&uid=' + userinfo.uid + '&pid=' + userinfo.pid;
